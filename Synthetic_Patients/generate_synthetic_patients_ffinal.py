...
Author: RAIDers ALS Subtyping Project - Kyulin Kim
...
def calculate_interaction_score(base_impact, ancestral_modifier, consequence_modifier):
    ...
    Interaction Score = (Base Impact × Ancestral Modifier × Consequence Modifier) + ε
    
    Where ε ~ N(0, 0.05) is the stochastic noise factor.
    This score determines the predicted disease progression.
    """
    score = base_impact * ancestral_modifier * consequence_modifier
    noise = np.random.normal(0, 0.05)  # Add ε ~ N(0, 0.05)
    return score + noise
...
def assign_progression_from_score(interaction_score):
    ...
    Based on Methodology Plan thresholds:
    - Score > 0.85: Fast Progression
    - Score 0.6-0.85: Slow Progression
    - Score ≤ 0.6: Asymptomatic / Low Penetrance
    """
    if interaction_score > FAST_PROGRESSION_THRESHOLD:
        return 'Fast Progression'
    elif interaction_score > SLOW_PROGRESSION_THRESHOLD:
        return 'Slow Progression'
    else:
        return 'Asymptomatic/Low Penetrance'
...
def main():
    ...
    for pop_idx, pop in enumerate(POPULATIONS):
        ...
        # STEP 4: Rare Variant Preservation (distributed across all populations)
        # Check at each population for variants with zero carriers and inject carriers
        # This ensures variant representation is balanced across populations
        ...
        # Calculate how many carriers to inject per population for balanced distribution
        # Each population handles a proportional share of the remaining needed carriers
        remaining_pops = len(POPULATIONS) - pop_idx
        for var_idx in variants_needing_carriers:
            needed_total = MIN_CARRIERS_PER_VARIANT - int(current_totals[var_idx])
            # Distribute needed carriers across remaining populations (ceiling division)
            needed_this_pop = max(1, (needed_total + remaining_pops - 1) // remaining_pops)
            non_carriers = np.where(genotype_matrix[:, var_idx] == 0)[0]
            if len(non_carriers) >= needed_this_pop:
                new_carriers = np.random.choice(non_carriers, size=needed_this_pop, replace=False)
